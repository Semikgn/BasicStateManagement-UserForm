...
fun MainScreen() {
    ...
    var email = remember { mutableStateOf("E-mail") }
    ...
    Column(
        ...
        //Kullanıcı adı için state oluşturulmuş textfield
        OzelTextField(string = kullaniciAdi.value, onValueChangeFunction = {
            kullaniciAdi.value = it
        })
        Spacer(modifier = Modifier.padding(10.dp))

        //Email için state oluşturulmuş textfield
        OzelTextField(string = email.value, onValueChangeFunction = {
            email.value = it
        })
        Spacer(modifier = Modifier.padding(10.dp))
...
fun OzelTextField(string: String, onValueChangeFunction : (String) -> Unit){
    TextField(value = string, onValueChange = onValueChangeFunction)
}
